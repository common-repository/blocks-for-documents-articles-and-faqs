...
class HTML_Elements {
			...
			<div class="radio-buttons-vertical <?php echo esc_html( $args['input_class'] )?>" id="<?php echo $id ?>">
				<ul>					<?php
					...
					foreach( $args['options'] as $key => $label ) {
						...
						if( $args['current'] == $key ) {
						    $checked_list = 'epbl-radio-checked';
						}						?>
	 ...
	 * @param $args
	 * $values:
	 * string $value['id']            ( Required ) Container ID, used for targeting with other JS
	 * string $value['type']          ( Required ) How it will look ( epbl-error = Red  )
	 * string $value['header']        ( Required ) The big Bold Main text
	 * HTML   $value['content']       ( Required ) Any HTML P, List etc...
	 * array  $value['checkboxes']    ( Optional ) Pass in an array of text labels.
	 */
	public function confirmation_box_with_checkboxes( $args ) { ?>

        <div class="epbl-notification-box">

            <form id="<?php echo $args['form_id']; ?>" class="<?php echo $args['type']; ?>">

                <section class="epbl-header">
                    <h3><?php echo $args['header']; ?></h3>
                </section>

                <section class="epbl-body">					<?php

                    if ( isset( $args['content'] ) ) {      ?>
                        <div class="epbl-body-content"><?php echo $args['content']; ?></div>                    <?php
                    }

					if ( isset( $args['checkboxes'] ) ) {   ?>
                        <div class="epbl-body-checkboxes">
                            <ul>
                ...
                <section class="epbl-footer">   <?php
					if ( isset ( $args['cancel_button'] ) ) {   ?>
                        <button id="epbl-notification-box-cancel" class="epbl-error-btn"><?php echo $args['cancel_button']; ?></button>       <?php
					}   ?>
                    <button id="epbl-notification-box-apply" class="epbl-success-btn"><?php echo $args['apply_button']; ?></button>
                </section>  <?php
	...
	public function confirmation_box_with_checkboxes_collapse( $args ) { ?>
        ...
        <div class="epbl-notification-box epbl-collapse">
            ...
            <form id="<?php echo $args['form_id']; ?>" class="<?php echo $args['type']; ?>">
                ...
                <section class="epbl-body">
                    <?php if ( isset( $args['content'] ) ) { ?>
                        <div class="epbl-body-content"><?php echo $args['content']; ?></div>
                        <?php }	?>
	                <div class="epbl-content-toggle"><?php echo $args['toggle_text']; ?><span class="epblfa epblfa-arrow-circle-down"></span> </div>
	 ...
	 * $values:
	 * string $value['id']            ( Required ) Container ID, used for targeting with other JS
	 * string $value['title']         ( Required ) The text while its collapsed
	 * HTML   $value['content']       ( Required ) Any HTML P, List etc...
	 */
	public function toggle_box( $args ) { ?>

        <div id="<?php echo $args['id']; ?>" class="epbl-toggle-box epbl-toggle-closed">

                <section class="epbl-toggle-box-header">
                    <h3>
                        <?php echo $args['title']; ?>
                        <span class="epbl-toggle-box-icon epblfa epblfa-arrow-circle-down"></span>
                    </h3>

                </section>

                <section class="epbl-toggle-box-body">
                    <div class="epbl-toggle-box-content"><?php echo $args['content']; ?></div>
                </section>
	 ...
	 * $values:
	 * string $value['id']            ( Required ) Container ID, used for targeting with other JS
	 * string $value['title']         ( Required ) The text title
	 * HTML   $value['content']       ( Required ) Any HTML P, List etc...
	 */
	public function info_box( $args ) {  ?>

        <span id="<?php echo $args['id']; ?>" class="epbl-info-box epbl-info-toggle-closed">

            <div class="epbl-info-box-icon epblfa epblfa-info-circle"></div>


            <div class="epbl-info-box-popup">
                <div class="epbl-info-box-container">
                    <span class="epbl-info-box-close epblfa epblfa-times"></span>
                    <span class="epbl-info-box-icon-max epblfa epblfa-expand"></span>
                    <span class="epbl-info-box-icon-min epblfa epblfa-compress"></span>
                    <section class="epbl-info-box-header">
                        <h3><?php echo $args['title']; ?></h3>
                    </section>
                    <section class="epbl-info-box-body">
                        <div class="epbl-info-box-content"><?php echo $args['content']; ?></div>
                    </section>
                </div>
            </div>
	 ...
	 * string $value['title']         ( Required ) The text while its collapsed
	 * string $value['callout_type']  ( Required ) The look of the box, Types include: success, attention, error, warning, info, default
	 * HTML   $value['content']       ( Required ) Any HTML P, List etc...
	 */
	...
	public function notification_box_basic( $args = array() ) {
		...
		$icon = '';
		switch ( $args['type']) {
			case 'error':   $icon = 'epblfa-exclamation-triangle';
				break;
			case 'success': $icon = 'epblfa-check-circle';
				break;
			case 'warning': $icon = 'epblfa-exclamation-circle';
				break;
			case 'info':    $icon = 'epblfa-info-circle';
				break;

		}		?>

		<div <?php echo isset($args['id']) ? 'id="'.$args['id'].'"' : ''; ?>class="epbl-notification-box-basic <?php echo 'epbl-notification-box-basic--'.$args['type']; ?>">

			<div class="epbl-notification-box-basic__icon">
				<div class="epbl-notification-box-basic__icon__inner epblfa <?php echo $icon; ?>"></div>
			</div>

			<div class="epbl-notification-box-basic__body">
				<h4 class="epbl-notification-box-basic__body__title"><?php echo $args['title']; ?></h4>
				<div class="epbl-notification-box-basic__body__desc"><?php echo $args['desc']; ?></div>
			</div>
