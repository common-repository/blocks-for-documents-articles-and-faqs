...
class KB_Search_Cntrl {
	...
	public function __construct() {
		add_action( 'wp_ajax_epbl-search-kb', array($this, 'search_kb') );
		add_action( 'wp_ajax_nopriv_epbl-search-kb', array($this, 'search_kb') );
	}
	...
	public function search_kb() {
		...
		$kb_id = Utilities::sanitize_get_id( $_GET['epbl_kb_id'] );
		if ( is_wp_error( $kb_id ) ) {
			wp_die( json_encode( array( 'status' => 'error', 'search_result' => esc_html__( 'Error occurred. Please try again later.', 'blocks-for-documents-articles-and-faqs' ), 'show_more' => false ) ) );
		}
		...
		$list_size = intval( Utilities::get( 'epbl_list_size' ) );
		...
		if ( empty($result) ) {
			$search_result = '<li class="epbl-sbsr__list-item"><div class="epbl-sbsr__list-no-results">' . $kb_config['no_results_found'] . '</div></li>';
			...
			foreach( $result as $post ) {
				...
				if ( has_filter( 'eckb_single_article_filter' ) ) {
					...
					$article_title_icon = empty( $article_title_icon ) ? 'epblfa-file-text-o' : $article_title_icon;
				}
				...
				$search_result .=
					'<li class="epbl-sbsr__list-item">' .
						'<a class="epbl-sbsr__list-item__inner" href="' .  esc_url( $article_url ) . '" ' . $new_tab . '>' .
							'<div class="epbl-sbsr__list-item__article">' .
								'<div class="epbl-sbsr__list-item__article__icon epblfa ' . esc_attr($article_title_icon) . '"></div>' .
								'<div class="epbl-sbsr__list-item__article__text">' . esc_html($post->post_title) . '</div>' .
							'</div>' .
						'</a>' .
					'</li>';
			}
